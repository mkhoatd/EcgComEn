...
OUTPUT_DIR = f'downsample_{MODE}_test_data'
...
def create_data(sig_names_with_mlii):
    for sig_name, sig_idx in sig_names_with_mlii:
        if not (sig_name in test_file_names):
            continue
        record = rdrecord(osp.join(MIT_DIR, sig_name))
        ...
        peak_pairs_np = peak_pairs_np[err_range < 1]
        labels = np.array(labels)[err_range < 1]
        for label, peak, true_peak in zip(labels, peak_pairs_np[:, 0], peak_pairs_np[:, 1]):
            if label == '/':
                label = 'PAB'
